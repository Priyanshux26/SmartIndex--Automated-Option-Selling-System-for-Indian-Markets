...
class KiteHistoricalDataDownloader:
    ...
    def download_instruments(self):
        ...
        try:
            ...
            if self._save_instruments_to_csv(filtered_instruments):
                self.logger.info(f"✓ Instrument download completed successfully")
        ...
        for attempt in range(self.PAGINATION_CONFIG['max_retries']):
            try:
                ...
                if attempt < self.PAGINATION_CONFIG['max_retries'] - 1:
                    ...
                    self.logger.error(f"  ✗ API call failed after {self.PAGINATION_CONFIG['max_retries']} attempts")
                    return []
    ...
    def download_historical_data_backward(self, instrument_token, tradingsymbol, exchange, interval='day'):
        ...
        try:
            ...
            self.logger.info(f"  📊 Date range: {date_ranges[-1][0].strftime('%Y-%m-%d')} to {date_ranges[0][1].strftime('%Y-%m-%d')}")
            ...
            self.logger.info(f"✓ Completed {tradingsymbol}: {total_records:,} total records from {successful_batches}/{len(date_ranges)} successful batches")
