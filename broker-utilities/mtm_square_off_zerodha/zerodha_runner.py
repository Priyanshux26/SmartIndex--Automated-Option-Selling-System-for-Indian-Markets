        ...
        while self.running:
            try:
                ...
                if current_time - last_detailed_status_log >= detailed_status_interval:
                    ...
                    # Process status
                    self.logger.info(f"🔧 Processes:")
                    self.logger.info(f"   LTP Subscriber: {'✅ Running' if health['ltp_subscriber']['running'] else '❌ Stopped'} "
                                     f"(PID: {health['ltp_subscriber']['pid']}, Restarts: {health['ltp_subscriber']['restart_count']})")
                    self.logger.info(f"   MTM Monitor: {'✅ Running' if health['mtm_monitor']['running'] else '❌ Stopped'} "
                    ...
                    if mtm_data.get('current_mtm') is not None:
                        ...
                        if discipline_status:
                            ...
                            self.logger.info(f"   Trading Discipline: {'🚫 ACTIVE (No new positions)' if discipline_active else '✅ INACTIVE (Trading allowed)'}")
