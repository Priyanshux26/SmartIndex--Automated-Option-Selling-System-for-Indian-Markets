...
class EnhancedMTMMonitor:
    ...
    def place_order_with_timeout(self, order_params: Dict) -> Tuple[Optional[str], Optional[str]]:
        ...
        try:
            self.logger.info(f"🔄 Placing order: {order_params['tradingsymbol']} | {order_params['transaction_type']} {order_params['quantity']}")
    ...
    def verify_order_execution_enhanced(self, order_id: str, expected_qty: int, symbol: str) -> Dict:
        ...
        try:
            ...
            while time.time() - start_time < self.order_verification_timeout and check_count < max_checks:
                ...
                try:
                    ...
                    if status != last_status:
                        ...
                        self.logger.info(f"📋 Order {order_id} status change: {last_status} → {status} | Filled: {filled_quantity}")
                        last_status = status
                    ...
                    if status in FINAL_STATUSES:
                        ...
                        if status == 'COMPLETE':
                            self.logger.info(f"✅ Order {order_id} COMPLETED in {elapsed:.2f}s - Filled: {filled_quantity}/{expected_qty}")
                            ...
                            self.logger.error(f"❌ Order {order_id} {status} in {elapsed:.2f}s - Reason: {status_message}")
                    ...
                    # Handle pending statuses
                        ...
                        if filled_quantity > 0:
                            ...
                            if fill_rate >= 80 and elapsed > (self.order_verification_timeout * 0.8):
                                self.logger.info(f"✅ Accepting partial fill for {order_id}: {fill_rate:.1f}% after {elapsed:.2f}s")
            ...
            # Timeout reached
            elapsed = time.time() - start_time
            self.logger.warning(f"⏰ Order verification timeout for {order_id} after {elapsed:.2f}s ({check_count} checks)")
            ...
            try:
                ...
                if final_status and len(final_status) > 0:
                    ...
                    if final_filled > 0:
                        self.logger.info(f"🟡 Timeout but found partial fill: {final_filled} qty, status: {final_status_text}")
    ...
    def place_square_off_order_with_verification(self, position: Dict) -> Dict:
        ...
        try:
            ...
            while remaining_quantity > 0 and (time.time() - square_off_start_time) < max_square_off_time:
                ...
                self.logger.info(f"📝 Placing order {order_sequence}/{((total_quantity - 1) // freeze_qty) + 1}: {action} {order_qty} of {tradingsymbol}")
                ...
                if placement_error:
                    ...
                    if retry_count < self.max_order_retries:
                        ...
                        self.logger.info(f"🔄 Retrying order {order_sequence} for {tradingsymbol} (attempt {retry_count + 1})")
                ...
                if execution_result['success']:
                    ...
                    self.logger.error(f"❌ Order {order_sequence} execution failed: {execution_result.get('reason', 'Unknown')}")
            ...
            self.logger.error(f"💥 Critical error in square-off for {position.get('tradingsymbol', 'Unknown')}: {e}")
            self.logger.error(f"💥 Stack trace: {traceback.format_exc()}")
        ...
        try:
            ...
            if sell_positions:
                ...
                for i, position in enumerate(sell_positions, 1):
                    self.logger.info(f"🔴 Processing SELL position {i}/{len(sell_positions)}: {position['tradingsymbol']}")
            ...
            if buy_positions:
                ...
                for i, position in enumerate(buy_positions, 1):
                    self.logger.info(f"🟢 Processing BUY position {i}/{len(buy_positions)}: {position['tradingsymbol']}")
            ...
            self.logger.critical("=" * 80)
            self.logger.critical(f"📊 RESULTS SUMMARY:")
        ...
        while True:
            try:
                ...
                if total_mtm <= self.mtm_threshold:
                    ...
                    self.logger.critical(f"🚨 LOSS THRESHOLD BREACHED! Current: ₹{total_mtm:.2f}, Threshold: ₹{self.mtm_threshold}")
                ...
                if should_square_off and self.auto_square_off and position_details:
                    ...
                    self.logger.critical(f"⚡ Positions squared: {square_off_result.get('total_success', 0)}/{square_off_result.get('total_positions', 0)}")

                    if square_off_reason in ["DAILY_TARGET_REACHED", "DISCIPLINE_MODE_ACTIVE"]:
                        self.logger.critical("🚫 TRADING DISCIPLINE ACTIVE: Any new positions will be immediately squared off!")
                ...
                if consecutive_errors >= max_consecutive_errors:
                    self.logger.critical(f"🚨 Too many consecutive errors. Stopping monitor.")
                    break
